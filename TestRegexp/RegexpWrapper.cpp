...
RegexpWrapper::RegexpWrapper(char* s)
{
	mState = MatchState(s);
}
...
void RegexpWrapper::match_callback(const char * match,          // matching string (not null-terminated)
	...
	const MatchState & ms)
{
	if ( ms.level > 0 )
	{
		...
		{
			char tmpStr[256] = { 0 };
			memcpy(tmpStr, ms.capture[i].init, ms.capture[i].len);
...
std::vector<std::string> RegexpWrapper::GlobalMatch(const char* expression)
{
	mResults.clear();
...
std::vector<std::string> RegexpWrapper::match(char* s, const char* expression)
{
	mResults.clear();
	mState.Target(s);
	mState.GlobalMatch(expression, &RegexpWrapper::match_callback);
